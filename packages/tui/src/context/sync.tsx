...
// Editor/IDE context pushed from the host editor via the `ide.context.updated`
// SDK event. Kept local to the TUI package (the old `@/ide` import doesn't
// belong across the extracted package boundary).
type IdeEditorContext = {
  uri?: string
  selection?: {
    ...
    event.subscribe((event, { workspace }) => {
      switch (event.type) {
        ...
        case "ide.context.updated": {
          setStore("ide_context", reconcile(event.properties as unknown as IdeEditorContext))
          break
        }
      }
    })

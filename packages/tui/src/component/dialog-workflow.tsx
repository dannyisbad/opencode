...
export function DialogWorkflow(props?: { openRunID?: string; openPhase?: string; openAgentID?: string }) {
  ...
  const [data, { refetch }] = createResource(async (): Promise<WorkflowData> => {
    ...
    return {
      ...
      runs: (runs.data ?? []).toSorted((a: WorkflowRun, b: WorkflowRun) => {
...
function DialogWorkflowRun(props: {
  ...
  createEffect(() => {
    ...
    const agentIndex = phases().findIndex((phase) =>
      current().agents.some((agent: WorkflowRun["agents"][number]) => agent.phase === phase),
    )
    const next =
      initialIndex >= 0
        ? initialIndex

...
if (!skipInstall) {
  const osFlag = allFlag ? "*" : "win32"
  await $`bun install --os=${osFlag} --cpu="*" @opentui/core@${pkg.dependencies["@opentui/core"]}`
  await $`bun install --os=${osFlag} --cpu="*" @parcel/watcher@${pkg.dependencies["@parcel/watcher"]}`
  await $`bun install --os=${osFlag} --cpu="*" @ff-labs/fff-bun@${pkg.dependencies["@ff-labs/fff-bun"]}`
}

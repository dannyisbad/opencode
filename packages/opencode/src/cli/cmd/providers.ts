...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const index = args.provider
      ? options.find(
          (option) =>
            credentials[option.value].key.toLowerCase() === args.provider.toLowerCase() ||
    ...
    const credential = index !== undefined ? credentials[index] : undefined

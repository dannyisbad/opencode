...
export function dynamicWorkflowPlannerPrompt(objective: string) {
  return [
    ...
    "- Distinct Roles: Assign clear, specialized roles to agents using the `agent` option in `ctx.agent` (e.g., 'explore' for research/finding files, 'build' for code changes/writing, and 'plan' or 'general' for synthesis/verification).",

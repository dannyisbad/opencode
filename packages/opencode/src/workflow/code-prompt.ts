...
export function codeWorkflowPlannerPrompt(objective: string): string {
  return [
    ...
    "- `await ctx.loop({ fn, until, maxIterations? })` -> `{ data, text }[]`. `fn(iteration, previous?)` returns an `AgentInput` ({prompt, agent?}); the engine runs it; `until(result, iteration)` returns true to stop. `maxIterations` is clamped to 1..20. Use for refine-until-good / loop-until-dry.",
    ...
    "- loop-until-dry: `ctx.loop` whose `until` stops after K rounds yield nothing new (respect the 1..20 clamp).",

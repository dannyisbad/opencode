...
// ── Fork port shim ────────────────────────────────────────────────────────────
// Upstream's `@/storage/db` exposed a SYNC module-global drizzle handle:
// `Database.use((db) => q.get())` returned rows directly. The fork's Database is
// an Effect service whose drizzle terminals (.get/.all/.run) return Effects.
// This shim keeps the upstream call shape — `Database.use(fn)` applies fn to the
// service handle — but the result is an Effect, so every call site yields it.
// The handle is stamped once at layer init (the layer requires the service).
type DbHandle = CoreDatabase.Interface["db"]
let dbHandle: DbHandle | undefined
const Database = {
  handle(): DbHandle {
    if (!dbHandle) throw new Error("Automation used before Database handle initialized")
    return dbHandle
  },
  // orDie: upstream's sync handle THREW on database failure (a defect). Keeping
  // that semantic here keeps the service Interface's error channels clean
  // (NotFoundError only), matching how the workflow engine treats DB failures.
  use<A, E, R>(fn: (db: DbHandle) => Effect.Effect<A, E, R>): Effect.Effect<A, never, R> {
    return fn(Database.handle()).pipe(Effect.orDie)
  },
  transaction<A, E, R>(
    fn: (tx: Parameters<Parameters<DbHandle["transaction"]>[0]>[0]) => Effect.Effect<A, E, R>,
  ): Effect.Effect<A, never, R> {
    return Database.handle().transaction(fn).pipe(Effect.orDie) as Effect.Effect<A, never, R>
  },
}
...
function patchRun(runID: AutomationRunID, patch: Partial<typeof AutomationRunTable.$inferInsert>) {
  // Effect.suspend so `Date.now()`/patch spread are evaluated at execution time,
  // matching the upstream Effect.sync(() => ...) laziness.
      ...
      automation.kind === "thread" && automation.threadID
        ? yield* sessions.get(automation.threadID).pipe(
            Effect.filterOrFail((session) => {
              ...
              if (automation.projectID === ProjectV2.ID.global) return session.directory === automation.directory
              return true
            }),
            Effect.option,
          )
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Stamp the module-level drizzle handle for the Database shim (see top of file).
    dbHandle = (yield* CoreDatabase.Service).db
    ...
    const remove = Effect.fn("Automation.remove")(function* (id: AutomationID) {
      ...
      yield* Database.use((db) => db.delete(AutomationTable).where(eq(AutomationTable.id, id)).run())
      return true
    })
    ...
    const enqueue = Effect.fn("Automation.enqueue")(function* (automation: Info) {
      const state = yield* InstanceState.get(scheduler)
      const active = yield* Database.use((db) =>
        db
          .select()
          .from(AutomationRunTable)
          .where(

...
describe("capability.isCodeCapable", () => {
  test("capable: anthropic opus/sonnet, gpt-5, o3/o4, gemini-3 pro+flash", () => {
  ...
  test("floor: haiku, mini, older/other flash, unknown → not capable", () => {
    expect(isCodeCapable(m("anthropic", "claude-haiku-4-5"))).toBe(false)
    // gemini-3 flash is trusted, but only on google and only at the gemini-3 gen:

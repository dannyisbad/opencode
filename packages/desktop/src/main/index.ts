...
const main = Effect.gen(function* () {
  ...
  // serverReady is declared below (after whenReady) with the fork's error
  // channel; the before-quit closure captures that binding — it only fires
  // long after initialization, so the later declaration is safe.
  ...
  registerIpcHandlers({
    ...
    // Route relaunch through the quit-confirm flow so running automations get
    // a chance to block it; finishQuit() handles the actual relaunch+quit.
